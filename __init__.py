...
def _build_system_prompt(
    ...
    if generate_negative:
        ...
        parts.append(
            "Do not include a negative prompt section, exclusion list, or headings such as 'Negative prompt:' inside the positive prompt. "
            "If the user explicitly asks to exclude something, preserve that constraint naturally inside the positive prompt rather than appending a separate negative section. "
            "Do not invent new negative clauses on your own."
        )
...
def _strip_negative_prompt_section(text):
    value = (text or "").strip()
    if not value:
        return ""
    # Remove explicit trailing negative-prompt sections that leak into the positive prompt.
    # Keep user-authored inline negatives elsewhere intact; only strip clearly labeled sections.
    patterns = [
        r"(?:^|\n|\r\n)\s*negative\s*prompt\s*:\s*.*$",
        r"(?:^|\n|\r\n)\s*negative\s*:\s*.*$",
        r"(?:^|\n|\r\n)\s*negative\s*prompts\s*:\s*.*$",
    ]
    cleaned = value
    for pat in patterns:
        cleaned = re.sub(pat, "", cleaned, flags=re.IGNORECASE | re.DOTALL)
    # Also handle a same-paragraph label at the end of a long prompt.
    cleaned = re.sub(r"(?is)(?:\s|[.])negative\s*prompt\s*:\s*.+$", "", cleaned)
    cleaned = re.sub(r"(?is)(?:\s|[.])negative\s*:\s*.+$", "", cleaned)
    return cleaned.strip().rstrip(" ,;")


def _clean_structured_value(text, allow_salvage=True, strip_negative_section=False):
    value = (text or "").strip()
    value = re.sub(r"\s*\[end of text\]\s*$", "", value, flags=re.IGNORECASE).strip()
    if strip_negative_section:
        value = _strip_negative_prompt_section(value)
    if not value:
        return "", False
    if _contains_output_scaffolding(value):
        if allow_salvage:
            ...
            if strip_negative_section:
                salvaged = _strip_negative_prompt_section(salvaged)
            if salvaged:
...
def _extract_json_payload(raw_output, raw_prompt, variant_count, generate_negative):
    try:
        ...
        cleaned, salvaged = _clean_structured_value(cleaned, allow_salvage=True, strip_negative_section=(not generate_negative))
        return cleaned, "", [cleaned] if cleaned else [], "legacy-salvaged" if salvaged else "legacy-fallback"

    parse_mode = "structured"
    final_prompt, final_salvaged = _clean_structured_value(payload.get("final_prompt", ""), allow_salvage=True, strip_negative_section=(not generate_negative))
    if final_salvaged:
    ...
    for item in raw_variants:
        cleaned, salvaged = _clean_structured_value(item, allow_salvage=True, strip_negative_section=(not generate_negative))
        if cleaned:
    ...
    negative = ""
    if generate_negative:
        negative, _ = _clean_structured_value(payload.get("negative_prompt", ""), allow_salvage=False, strip_negative_section=False)
...
def _candidate_rank(raw_prompt, final_prompt, variants, detail_strength, length_target, aesthetic, variant_count=1, generate_negative=False):
    """Higher is better. Never let an empty/worse retry replace a usable draft."""
    issues = _rewrite_quality_issues(
        raw_prompt, final_prompt, variants, detail_strength, length_target, aesthetic, variant_count, generate_negative
    )
    ...
    target_chars = LENGTH_MIN_CHARS[length_target]
    meets_length = int(words >= target_words and chars >= target_chars)
    useful_words = min(words, target_words + 160)
    # For Long/Max Detail, a clean candidate that actually reaches the selected
    # length floor must always outrank a shorter draft. This prevents a polished
    # 50-word answer from winning merely because it has fewer soft warnings.
    if length_target in ("Long", "Max Detail"):
        return (meets_length, -penalty, useful_words, chars), issues
    return (-penalty, useful_words, chars), issues


def _rewrite_quality_issues(raw_prompt, final_prompt, variants, detail_strength, length_target, aesthetic, variant_count=1, generate_negative=False):
    if not (final_prompt or "").strip():
    ...
    if (not generate_negative) and any(re.search(r"\bnegative\s*prompt\s*:", p or "", flags=re.IGNORECASE) for p in prompts):
        issues.append("negative-prompt section leaked into the positive prompt")
...
class RebelsPromptEnhancer:
    ...
    def enhance(self, raw_prompt, purpose, model_format, aesthetic, precision,
        ...
        quality_retries = 0
        best_rank, quality_issues = _candidate_rank(raw_prompt, final_prompt, variants, detail_strength, length_target, aesthetic, variant_count, generate_negative)
        ...
        max_quality_retries = 5 if length_target in ("Long", "Max Detail") else 3
        while quality_issues and quality_retries < max_quality_retries:
            quality_retries += 1
            issue_text = "; ".join(quality_issues)
            best_text_for_repair = (best_candidate[0] or "").strip()
            repair_sys_prompt = sys_prompt + (
                f" QUALITY RETRY {quality_retries}: The best draft so far was rejected because: {issue_text}. "
                f"The selected length is {length_target}. The completed positive prompt should reach at least "
                f"{LENGTH_MIN_WORDS[length_target]} useful words and {LENGTH_MIN_CHARS[length_target]} characters. "
                "Do not stop after a summary paragraph. Expand the scene with NEW, non-repetitive concrete visual information: "
                "subject design and condition, action/pose, environment, foreground/midground/background, spatial relationships, "
                "composition, viewpoint, lens/camera behavior, lighting direction and quality, materials/textures, color palette, "
                "atmosphere, depth cues, and model-appropriate rendering language where relevant. "
                "Do not pad with synonyms, duplicated phrases, generic quality buzzwords, or invented contradictions. "
                ...
                "Keep every explicit user fact and obey the selected aesthetic as a hard requirement. "
                + (f"Use this existing draft as the content foundation and EXPAND it rather than replacing it with a shorter summary: {best_text_for_repair} " if best_text_for_repair else "") +
            ...
            retry_raw, retry_gpu_layers, retry_backend = _run_rewrite(
                ...
                n_ctx=effective_ctx,
                max_tokens=max_tokens,
                temperature=(0.58 if length_target in ("Long", "Max Detail") else min(0.84, 0.74 + 0.03 * quality_retries)),
                top_p=0.92,
                repeat_penalty=1.10,
        ...
        # Structural contamination is fatal. Length is NEVER fatal:
        # after the dedicated expansion retries, return the best clean draft with
        # a quality warning rather than breaking the user's ComfyUI workflow.
