...
# Quality floors: lower than the requested target ranges on purpose. They reject
# clearly under-developed output without forcing filler or exact word counts.
LENGTH_MIN_CHARS = {"Short": 80, "Medium": 260, "Long": 650, "Max Detail": 950}
LENGTH_MIN_WORDS = {"Short": 24, "Medium": 60, "Long": 125, "Max Detail": 180}
...
def _run_portable_rewrite(
    ...
    """Safe out-of-process fallback for broken/incompatible in-process llama DLLs.

    NVIDIA/Windows systems prefer the official CUDA 12.4 build and offload all
    possible layers. If CUDA cannot initialize/run, the existing CPU runtime is
    used automatically as a safety net.
    """
    prompt = (
        sys_prompt.strip()
        + "\n\nUSER REQUEST:\n"
    ...
    if os.name == "nt" and torch.cuda.is_available():
        try:
            cuda_exe = _ensure_portable_cuda_runtime()
            if cuda_exe is not None:
                cmd = _portable_completion_cmd(
                    cuda_exe, model_path, prompt, n_ctx, max_tokens,
                    temperature, top_p, repeat_penalty, seed, schema_json,
                    gpu_layers=999,
                )
                proc = _run_portable_process(cmd, cuda_exe, timeout_seconds=300)
                stdout, detail = _portable_failure_detail(proc)
                if proc.returncode == 0 and stdout:
                    return stdout
                cuda_error = detail
                print(
                    "[RebelsPromptEnhancer] Portable CUDA llama.cpp failed; "
                    "falling back to portable CPU runtime. " + detail[-800:]
                )
        except Exception as exc:
            cuda_error = str(exc)
            print(
                "[RebelsPromptEnhancer] Portable CUDA llama.cpp unavailable; "
                "falling back to portable CPU runtime. " + cuda_error[-800:]
            )

    cpu_exe = _ensure_portable_cpu_runtime()
    cmd = _portable_completion_cmd(
        cpu_exe, model_path, prompt, n_ctx, max_tokens,
        temperature, top_p, repeat_penalty, seed, schema_json,
        gpu_layers=0,
    )
    try:
        proc = _run_portable_process(cmd, cpu_exe, timeout_seconds=900)
    except Exception as exc:
        if cuda_error:
            raise RuntimeError(
                "Portable llama.cpp CUDA path failed, then CPU fallback also failed: "
    ...
    if proc.returncode != 0 or not stdout:
        if cuda_error:
            detail = f"CUDA: {cuda_error[-1200:]} | CPU: {detail}"
...
def _schema_for_rewrite(variant_count, generate_negative, min_prompt_chars=1):
    # Keep grammar constraints structural only. V3.2.9 tried to encode Long/Max
    # length floors as huge JSON-string minLength values; smaller Qwen quants can
    # satisfy that grammar by stuffing template/schema text into the string. Length
    # is therefore validated semantically after generation instead.
...
def _max_tokens_for_length(length_target, base=None, variant_count=1, generate_negative=False):
    ...
    per_variant = {"Short": 260, "Medium": 520, "Long": 900, "Max Detail": 1400}[length_target]
    if base is not None:
    ...
    def enhance(self, raw_prompt, purpose, model_format, aesthetic, precision,
        ...
        while quality_issues and quality_retries < 2:
            ...
            repair_sys_prompt = sys_prompt + (
                f" QUALITY RETRY {quality_retries}: The previous draft was rejected because: {issue_text}. "
                f"Write a genuinely {length_target.lower()} prompt with substantial, concrete visual detail; aim for at least "
                f"about {LENGTH_MIN_WORDS[length_target]} useful words without padding or repetition. "
                ...
                "Return exactly one clean JSON object and nothing else."
            )
            raw_output, used_gpu_layers, backend_used = _run_rewrite(
                model_path=model_path,
                raw_prompt=raw_prompt,
                ...
                n_ctx=effective_ctx,
                max_tokens=max_tokens,
                temperature=min(0.82, 0.74 + 0.04 * quality_retries),
                top_p=0.92,
                repeat_penalty=1.06,
        ...
        if quality_issues:
            raise RuntimeError(
                "Prompt enhancer could not satisfy the selected controls after automatic repair attempts: "
                + "; ".join(quality_issues)
            )
